...
*outcomes 是什么意思
what is tuple and what is feature of a tuple
```
...
# The Game of Hog [hog.zip](https://cs61a.org/proj/hog/hog.zip)
...
>
> - Submit with Phase 1 complete by **Thursday 09/11**, worth 1 pt.
> - Submit the complete project by **Thursday 09/18**.
>
...
>
> You can get 1 bonus point by submitting the entire project by **Wednesday 09/17**. You can receive extensions on the project deadline and checkpoint deadline, but not on the early deadline, unless you're a DSP student with an accommodation for assignment extensions.

In this project, you will develop a simulator and multiple strategies for the dice game Hog. You will need to use *control statements* and *higher-order functions* together, as described in Lectures 1-4 and Sections 1.2 through 1.6 of [Composing Programs](https://www.composingprograms.com/), the online textbook.
...
- **Sus Fuss**. We call a number [*sus*](https://en.wikipedia.org/wiki/Sus_(genus)) if it has exactly 3 or 4 factors, including 1 and the number itself. If, after rolling, the current player's score is a sus number, their score instantly increases to the closest prime number greater than their current score.
...
The project is worth 25 points, of which 1 point is for submitting Phase 1 by the checkpoint date of Thursday 09/11.
...
Implement the `roll_dice` function in `hog.py`. It takes two arguments: a positive integer called `num_rolls`, which specifies the number of times to roll a die, and a `dice` function. It returns the number of points scored by rolling the die `num_rolls` times in a turn: either the sum of the outcomes or 1 *(Sow Sad)*.
...
You can also test `boar_brawl` interactively by running `python3 -i hog.py` from the terminal and calling `bo	ar_brawl` on various inputs.
...
```
  ...
  elif is_prime(n) == True: # 为什么is_prime() 里不带值也不会报错？ 
...
简单解释： 在函数列表中使用： 创建一个函数的列表，可以直接在列表中使用 lambda 表达式。 functions = [lambda x: x + n for n in range(5)] for f in functions: print(f(0)) # 输出 0, 1, 2, 3, 4

edit

GOAL = 100 # The goal of Hog is to score 100 points. 这个大写是有什么意义吗
...
Once you are satisfied, submit the checkpoint by uploading `hog.py` to the Hog Checkpoint assignment **on Pensieve.** Be sure to submit before the checkpoint deadline of Thursday 09/11. For a refresher on how to submit to Pensieve, refer to [Lab 00](https://cs61a.org/lab/lab00/#task-c-submitting-the-assignment).

You can add a partner to your Pensieve submission by clicking on **Edit Group** and entering their email address. Only one partner needs to submit to Pensieve.
...
Once you are satisfied, submit this assignment by uploading `hog.py` **to Pensieve.** For a refresher on how to do this, refer to [Lab 00](https://cs61a.org/lab/lab00/#task-c-submitting-the-assignment).
...
If you're interested, you can take your implementation of Hog one step further by participating in the completely optional [Hog Strategy Contest](https://cs61a.org/proj/hog_contest), where you design a `final_strategy` to be played against those of other students. The winning strategies will receive a paltry amount of extra credit.
